...
json::Value Proxy::request(
  ...
  if (verb == "POST") {
    ...
    curl_handle.setOpt(new cURL::Options::Put(true));
    curl_handle.setOpt(new cURL::Options::ReadStream(&request));
    curl_handle.setOpt(new cURL::Options::PostFieldSize(fdata.length()));
  } else if (verb == "DELETE") {
  ...
  if (debug) {
    ...
    if (!fdata.empt()) std::cerr << "SEND BODY: " << fdata << std::endl;
  }
